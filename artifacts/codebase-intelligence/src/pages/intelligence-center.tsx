...
export default function IntelligenceCenter() {
  ...
  const ask = () => {
    ...
    if (q.includes("security") || q.includes("vulnerab") || q.includes("secret")) return setAnswer(`${securityFindings.length} security findings were detected by ${security?.rulesScanned ?? 0} static rules across ${security?.filesScanned ?? 0} files. ${securityFindings.slice(0, 3).map((r) => `${r.title} in ${r.file}:${r.line}`).join("; ") || "No findings were detected by the current rule set."}`);
